...
class Index extends Component {
  ...
  render() {         //这里面输出props看看里面有什么东西
    console.log('modelindex');
    console.log(this.props);
    let {state} = this.props;
    return (<div style={styles.circle}>
      <div style={{color: state.Theme.color}}>{state.Increase.count}</div>
      <div style={styles.btnTheme} onClick={this._ThemeHandle.bind(this)}>切换主题</div>
      <div style={styles.btnAddOne} onClick={()=> {
        this._addHandle(1)
      }}>+1
      </div>
      <div style={styles.btnAddTwo} onClick={()=> {
        this._addHandle(2)
      }}>+2
      </div>
    </div>         )
